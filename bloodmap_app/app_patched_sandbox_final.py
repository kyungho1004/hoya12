...
# --- v6 sandbox: enforce unique keys & prevent recursive monkeypatch ---
...
def _run_special_ui_sandbox(ui_func):
    ...
    # Safe wrappers that *always* use originals (no re-entrancy) and mint keys if missing
...
# Replace global name so existing calls use the sandbox
def special_tests_ui(*args, **kwargs):
    if _special_tests_ui_orig is None:
        try:
            ...
            pass
        st.session_state["special_interpretations"] = []
        return []
    return _run_special_ui_sandbox(_special_tests_ui_orig)

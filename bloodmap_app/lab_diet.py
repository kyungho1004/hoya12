...
def lab_diet_guides(labs: Dict, heme_flag: bool = False) -> List[str]:
    ...
    if ANC is not None and 500 <= ANC < 1000:
        add("ANC 500~1000 (주의)", ["흰죽","계란찜(완숙)","연두부","익힌 야채","잘 익힌 고기/생선"],
            caution="생채소는 가급적 피하고, 모든 음식은 완전히 익히기 또는 전자레인지 30초↑ · 멸균/살균 식품 권장 · 남은 음식은 2시간 지나면 폐기 · 껍질 있는 과일은 주치의와 상의")
    if ANC is not None and ANC >= 1000:
        add("ANC ≥1000 (완화)", ["일반 부드러운 식단","계란(완숙)","잘 익힌 육류/생선","익힌 채소","과일(철저 세척)"],
            caution="기본 위생 수칙 준수 · 생고기/생선/반숙/회/생채소는 아직 피하기 권장(의료진이 허용 시 점진적 확대)")

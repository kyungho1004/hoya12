...
# ---- Safe branding bootstrap (handles optional args) ----
def _safe_call(obj, fname, **kws):
    ...
    try:
        ...
        filtered = {k:v for k,v in kws.items() if k in sig.parameters}
        return fn(**filtered) if filtered else fn()
    except Exception as e:
        st.warning(f"{fname} 호출 건너뜀: {e}")

def safe_branding_boot():
    import streamlit as st
    app_url = ""
    made_by = "피수치 홈페이지 프로젝트"
    try:
        app_url = st.secrets.get("APP_URL", app_url)
        made_by = st.secrets.get("MADE_BY", made_by)
    except Exception:
        pass
    _safe_call(branding, "render_deploy_banner", app_url=app_url, made_by=made_by)
    _safe_call(branding, "render_header")

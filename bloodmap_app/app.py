...
with st.sidebar:
    ...
    pin_field = st.text_input("PIN 숫자 (별명만 입력했다면 여기에)", value=st.session_state.get("_pin_raw",""), key="_pin_raw", type="password", help="숫자 4~8자리")
    # PIN 추출
    if "#" in raw_key:
    ...
    if need_auth:
        st.warning("PIN 재인증 필요(기능 사용 가능). 숫자 4~8자리 입력 후 [PIN 인증]을 눌러 주세요.")
    else:
...
with t_peds:
    ...
    # 아데노바이러스 의심 가중
    if (max_temp is not None and max_temp >= 39.0) and (eye in ["노랑-농성", "양쪽"]) and ((cough in ["보통","심함"]) or (stool in ["1~2회","3~4회","5~6회","7회 이상"])):
        score["아데노바이러스 의심"] += 60

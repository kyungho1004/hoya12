...
def main():
    ...
    if mode == "소아 일상/질환 모드":
        ...
        if st.button("해석하기"):
            derived = {}
            albumin_u = st.session_state.get("요 알부민 (mg/L)") if "요 알부민 (mg/L)" in st.session_state else None
            urine_cr = st.session_state.get("요 크레아티닌 (mg/dL)") if "요 크레아티닌 (mg/dL)" in st.session_state else None

...
def _carelog_24h_totals(log=None):
    ...
    total_apap = 0.0
    total_ibu = 0.0
    last_apap_epoch = None
    last_ibu_epoch = None

    def _to_epoch(ts_val):
        """문자열/naive/tz-aware 모두 KST epoch초로 변환."""
        try:
            from datetime import datetime as __dt
            if isinstance(ts_val, str):
                try:
                    d = __dt.fromisoformat(ts_val)
                except Exception:
                    d = __dt.fromisoformat(ts_val[:19])  # 'YYYY-MM-DD HH:MM:SS'
            elif isinstance(ts_val, __dt):
                d = ts_val
            else:
                return None
            if d.tzinfo is None:
    ...
    for rec in (log or []):
        tsec = _to_epoch(rec.get('ts'))
        if tsec is not None and tsec < since_epoch:
            continue
        ...
        if any(k in drug for k in ("apap","acetaminophen","타이레놀")):
            total_apap += mg
            if tsec is not None and (last_apap_epoch is None or tsec > last_apap_epoch):
                last_apap_epoch = tsec

        if any(k in drug for k in ("ibu","ibuprofen","이부프로펜")):
            total_ibu += mg
            if tsec is not None and (last_ibu_epoch is None or tsec > last_ibu_epoch):
                last_ibu_epoch = tsec

    last_apap_dt = _dt.fromtimestamp(last_apap_epoch, _KST) if last_apap_epoch is not None else None
    last_ibu_dt  = _dt.fromtimestamp(last_ibu_epoch,  _KST) if last_ibu_epoch  is not None else None

    return dict(
        now=now_dt,
        total_apap=total_apap,
        total_ibu=total_ibu,
        last_apap=last_apap_dt,
        last_ibu=last_ibu_dt
    )

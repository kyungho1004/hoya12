...
def main():
    ...
    if "schedules" not in st.session_state:
        ...
        if mode == "일반/암":
            ...
            _show_labs = st.checkbox("🧫 피수치 입력 열기", value=False, help="소아 모드에서는 필요 시에만 열어 입력")
        if _show_labs:
            st.markdown("### 🧫 기본 피수치 입력")
            _fields = [
                ("WBC", f"{LBL_WBC} (백혈구)", 1),
                ("Hb",  f"{LBL_Hb} (혈색소)", 1),
                ("PLT", f"{LBL_PLT} (혈소판)", 0),
                ...
                ("Na",  "나트륨(Na) (mEq/L)", 0),
                ("K",   "칼륨(K) (mEq/L)", 1),
                ("Alb", "알부민(Albumin) (g/dL)", 1),
                ("Glu", "혈당(Glucose) (mg/dL)", 0),
                ("TP",  "총단백(TP) (g/dL)", 1),
                ("AST", "AST (U/L)", 0),
                ("ALT", "ALT (U/L)", 0),
                ("LDH", "LDH (U/L)", 0),
            ...
            _cols = st.columns(3)
            for _i, (_k, _label, _dec) in enumerate(_fields):
                with _cols[_i % 3]:
                    vals[_k] = num_input_generic(_label, key=f"lab_{_k}", decimals=_dec)
            if entered(vals.get("Ca")) and entered(vals.get("Alb")):
                _corr = calc_corrected_ca(vals.get("Ca"), vals.get("Alb"))
                if _corr is not None:
                    st.caption(f"보정 칼슘: {_corr} mg/dL (Ca 보정식 적용)")
        else:
            vals = {}
    ...
    # === 암 모드: 항암제/항생제 입력 통합 UI ===def _get_drug_list():

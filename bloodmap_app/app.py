...
# ---------- App Header ----------# ===== 안정패치: 진단 토글 + 전역 예외 가드 =====
import traceback
from contextlib import contextmanager

SHOW_DIAG = st.sidebar.toggle("🔧 진단 모드(에러 표시/로그)", False, key="bm_diag_toggle")
...
with t_peds:
    ...
    # ---------- 특수검사(자리표시자) ----------
with t_special:
    with safe_block("t_special"):
        st.markdown("#### 특수검사")
        st.write("특수검사 해석 모듈과 연동할 수 있습니다.")
    
    # ---------- 보고서 + 응급도 평가 ----------
with t_report:
    with safe_block("t_report"):

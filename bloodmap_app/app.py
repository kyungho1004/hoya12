    ...
    *,
    ...
    if stool in ["3~4회", "5~6회", "7회 이상"]:
        bullet(
            "💧 설사/장염 의심",
            """
st.markdown("- 하루 **3회 이상 묽은 변** → 장염 가능성")
st.markdown("- **노란/초록 변**, **거품 많고 냄새 심함** → 로타/노로바이러스 고려")
...
- **즉시 진료**: 숨차함/청색증/가슴함몰
    ...
    if eye in ["노랑-농성", "양쪽"]:
        bullet(
            ...
            """
- 손 위생 철저, 분비물은 깨끗이 닦기
st.markdown("- ****양쪽·고열·눈 통증/빛 통증**** → 진료 권장")
            """,
        )
    if abd_pain:
        bullet(
            "😣 복통/배 마사지 거부",
            """
st.markdown("- 우하복부 통증·보행 악화·구토/발열 동반 → **충수염 평가**")
st.markdown("- 혈변/흑변 동반 → **즉시 진료**")
            """,
        )
    if ear_pain:
        bullet(
            ...
            """
- 눕기 불편 시 **머리 살짝 높이기**
st.markdown("- 38.5℃↑, 지속 통증, **귀 분비물** → 진료 필요")
            """,
        )
    if rash:
        bullet(
...
st.markdown("- **호흡곤란/쌕쌕/목 조임** → **즉시 응급실**")
            """,
        )
    if migraine:
        bullet(
...
- 어두운 곳 휴식, 수분 보충
st.markdown("- **번개치듯 새로 시작한 극심한 두통**/신경학적 이상 → 응급평가")
            """,
        )
    if hfmd:
        bullet(
...
- **손·발·입 안** 물집/궤양 + 발열
- 전염성: 손 씻기/식기 구분
st.markdown("- **탈수(소변 감소·축 늘어짐)**, **고열 >3일**, **경련/무기력** → 진료 필요")
            """,
        )
...
st.markdown("- **복부 압통 심함/우하복부 통증 지속**, 보행 시 통증 악화 → **의료진 상담**.")
- **금식이 필요한 경우**가 있으므로, **구토/심한 통증** 시 **음식은 일단 중단**하고 수분 소량씩.
- 진통제는 **의사 지시에 따라** 사용(복통 원인 가리기 주의).
st.markdown("- **경고 신호**: 지속 악화, 복부 팽만, **혈변/흑색변**, **발열 ≥ 38.5 ℃**, **탈수** → 즉시 병원.")
        """.strip()
    )
...
st.markdown("- **탈수 신호**: 소변량 감소, 입마름, 눈물 감소, 기운 없음 → 병원 상담.")
...
st.markdown("- **고열(≥ 38.5 ℃)** 지속/호흡 곤란/가슴 통증/청색증(입술 파래짐) → 즉시 병원.")

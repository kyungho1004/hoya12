...
# ---------------------------------
# Feedback Bundle (self-contained)
# ---------------------------------
def _feedback_dir():
    for p in [
...
def set_current_tab_hint(name: str) -> None:
    """Stores a page hint for logging (safe if unused elsewhere)."""
    st.session_state["_bm_current_tab"] = name

def render_feedback_box(default_category: str = "일반 의견", page_hint: str = "") -> None:
    """Feedback form with unique widget keys per page_hint to avoid duplicate key errors."""
...
def render_feedback_admin(key_suffix: str = "admin") -> None:
    """Admin view with unique key per location to avoid duplicate element key errors."""
    pwd = st.text_input("관리자 비밀번호", type="password", key=f"fb_admin_pwd_{key_suffix}")
...
def attach_feedback_sidebar(page_hint: str = "Home") -> None:
    """Attach a sidebar feedback form; internally uses 'Sidebar' suffix for uniqueness."""
    with st.sidebar:
        st.markdown("### 💬 의견 보내기")

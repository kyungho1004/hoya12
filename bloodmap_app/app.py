...
cancer = st.selectbox("혈액암(진단명)", heme_display)
    ...
    if mode == "일반/암" and group and group != "미선택/일반" and cancer:
        ...
        heme_by_cancer = {
            ...
            "APL": ["ATRA","Arsenic trioxide","Idarubicin","Daunorubicin","ARA-C","MTX","6-MP","G-CSF"],
    ...
    if mode == "일반/암":
        ...
        spo2_na = st.checkbox("산소포화도 측정기 없음/측정 불가", key="ped_spo2_na", value=True)
        if not spo2_na:
    ...
    if run:
        ...
        report_md += "\n\n> ⚠️ 본 수치는 참고용이며 개발자와 무관하며, 수치 기반 임의조정은 금지입니다. 반드시 의료진과 상의 후 결정하시기 바랍니다.\n"

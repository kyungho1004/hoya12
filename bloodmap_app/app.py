...
# ---- PIN status badge (sidebar) ----
def render_pin_status_badge():
    import streamlit as st
    pin_val = st.session_state.get("_pin", "")
    with st.sidebar:
        st.markdown("### 🔒 PIN 상태")
        if isinstance(pin_val, str) and pin_val.isdigit() and 4 <= len(pin_val) <= 8:
            st.caption("✅ PIN 확인됨")
        else:
            st.caption("⚠️ PIN 미확인 — 사이드바 '별명#PIN' 형식(예: user#1234) 또는 숫자 4~8자리 입력")

...
- **구토시 등을 손바닥이 아닌 계란잡듯이 해서 몸을 70정도로 숙여서 두드려주시면 기도확보에 도움이됩니다.
            """,
        )
    if oliguria:
        bullet(
...
-** 탈수증상이 의심이 집에서 최대한 수분보충해주시고 그래도 호전이없다면 빠른 병원진료를 권장합니다
            """,
        )
...
- **즉시 진료**: 숨차함/청색증/가슴함몰
- **기침시 섹섹거림 등은 모세기관지염일수도 있으니 병원에서 정확한 진료를 보시기를 권장드리며
- 만약 섹섹거림없이 기침가 콧물이 난다면 생리식염수로 흡입기 치료해주시고 머리르 높여주시면 도움됩니다
            """,
        )
    if eye in ["노랑-농성", "양쪽"]:
        bullet(
            ...
            """
- 손 위생 철저, 분비물은 깨끗이 닦기
- *눈충혈 양쪽눈에 눈꼽이 끼는등 증상이 일어날수있습니다
- *만약 눈충혈없이 눈꼽이 계속해서 끼고 기침을하고 고열이 난다면
- *아데노 바이러스 있으니 빠른 근처 소아과 진료를 권장드립니다
            """,
        )
    if abd_pain:
        bullet(
...
- 복부에 팽창감 옆으로 옆구리 통증 구토 발열 이 동반되면 충수염일수도있으니 
- 잘 지켜봐주시면서 필요시 119 나 응급실 내원을 권장드립니다
- 혈변/흑변 동반 → **119나 응급실내원을 권장드립니다**
            """,
        )
    if ear_pain:
        bullet(
...
- 중이염 의심시 귀에 물이 들어가지않게 조심하여 씻겨주시고
- 만약 목욕을 시켜야한다면 귀에 물이 들어자않도록 최대한 조심하시기바랍니다
            """,
        )
    if rash:
        bullet(
            ...
            """
- **미온 샤워**, 면 소재 옷, 시원한 로션
- 새로운 로션이나 바디워시등을 사용 하였는지 확인하여주시고
- 만약 너무 심하거나 온몸에 올라온다면 아나필릭시스가 일어날수있으니 
- 가까운 응급실이나 119에 연락하여 병원 내원을 권장드립니다
            """,
        )
    if hives:
        bullet(
...
- 어두운 곳 휴식, 수분 보충
- 편두통이 지속된다면 우선 구토 나 어지러움증이 동반된다면 신경학적 이상일수있습니다.
...
- 수족구 확인방법은 투명한 컵으로 혀바닥에 빨간색 혈점 살살 눌러서 사라지면 수족구가 아닐수도있습니다
- 수족구 발생시 투명한 컵으로 살짝 눌러도 사지지않으며 유지됩니다 아이의 혀바닥에는 돌기와 혈점이 존재하므로 
- 혼돈하지않는게 도움이됩니다
            """,
        )

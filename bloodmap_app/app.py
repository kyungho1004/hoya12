# -*- coding: utf-8 -*-
# =========================================================
# BloodMap (피수치 가이드) - 단일 파일 앱
# 제작: Hoya/GPT | 자문: Hoya/GPT
# 안내: 세포/면역치료(CAR-T, TCR-T, NK, HSCT 등)는 표기하지 않습니다.
# =========================================================

"""
이 파일은 두 가지 실행 모드를 지원합니다.
1) Streamlit UI 모드: 환경에 streamlit이 설치된 경우 대화형 웹앱으로 실행
2) CLI(콘솔) 폴백 모드: streamlit 미설치 환경에서도 임포트/실행 가능하도록 보고서 생성/자가테스트만 수행

▶ 오류 "ModuleNotFoundError: No module named 'streamlit'"가 발생하는 경우,
   이 파일은 자동으로 CLI 폴백 모드로 동작합니다(테스트 실행 + 샘플 리포트 파일 생성).
"""

import io, json, uuid, base64, math, os, sys
...
def peds_diet_guide(disease=None, vals=None) -> Tuple[List[str], List[str], List[str]]:
    """
    Returns: foods, avoid, tips (각각 list[str])
    정책: 1회 용량(ml)만 표기. '1일 최대'는 표기하지 않음.
    """
    disease = disease or {}
    vals = vals or {}
    foods: List[str] = []
...
DRUGS = {
    ...
    "육종 - Osteosarcoma(골육종)": {
        ...
        "표적치료제": [],
        "항생제": [
            {"name": "Cefepime (세페핌)", "moa": "광범위", "se": "발진/설사"},
        ],
    },
    "희귀암 - GIST": {
        "항암제": [],
...
def interpret_labs(vals: Dict[str, Optional[float]]) -> List[str]:
    ...
    if (wbc := v.get("WBC")) is not None:
        if wbc < 2:
            add("WBC", "매우 낮음 → 감염 위험", "danger")
        elif wbc < 4:
            add("WBC", "낮음 → 감염 주의", "warn")
        elif wbc <= 10:
            add("WBC", "정상 범위", "ok")
        else:
            add("WBC", "높음 → 감염/염증 가능", "warn")
    ...
    if (plt := v.get("PLT")) is not None:
        if plt < 20:
            add("혈소판", "매우 낮음 → 출혈 위험", "danger")
        elif plt < 50:
            add("혈소판", "낮음 → 멍/출혈 주의", "warn")
        elif plt < 150:
            add("혈소판", "경계", "warn")
        else:
            add("혈소판", "대체로 양호", "ok")
    ...
    if (anc := v.get("ANC")) is not None:
        if anc < 500:
            add("ANC", "중증 호중구감소(격리/식이 위생 엄격)", "danger")
        elif anc < 1000:
            add("ANC", "호중구감소(감염 주의)", "warn")
        else:
            add("ANC", "대체로 양호", "ok")
    ...
    if (bnp := v.get("BNP")) is not None:
        if bnp >= 100:
            add("BNP", "상승(심기능 저하 가능성, 임상과 함께)", "warn")
        else:
            add("BNP", "대체로 양호", "ok")
...
FOOD_EXAMPLES = {
    ...
    "Ca_low": ["연어 통조림", "두부", "케일", "브로콜리", "참깨(대체: 아몬드)"],
}
...
def run_streamlit_app():
    from streamlit.components.v1 import html  # 늦은 임포트(폴백 안전)
    ...
    if mode == "소아 일상/질환":
        ...
        if foods:
            st.markdown("- **예시 음식**: " + ", ".join(foods))
        if avoid:
            st.markdown("- **피해야 할 예시**: " + ", ".join(avoid))
        if tips:
            st.markdown("**참고 팁**")
            for x in tips:
                st.markdown(f"- {x}")
        copy_button("예시 음식: " + ", ".join(foods) + "\n피해야 할 예시: " + ", ".join(avoid) + "\n" + "\n".join(tips), "📋 식이가이드 복사")

        with st.expander("🧪 (선택) 피수치 입력", expanded=False):
            st.caption("선택 시에만 입력란을 보여줍니다.")
            st.text_input("WBC (백혈구)")
            st.text_input("Hb (혈색소)")
            st.text_input("혈소판")
            st.text_input("CRP")
            st.text_input("ANC (호중구)")
    ...
    # 암 진단 모드 ---------------------------------------------------------
        ...
        with st.expander("💊 보기용 약제 제안 (자동)", expanded=False):
            ...
            with c3:
                ...
                with st.expander("공통 목록 (항생제/항진균/스테로이드)", expanded=False):
                    ...
                    blk = []
                    for d in COMMON_ABX:
                        blk.append(f"{d['name']} | 작용:{d['moa']} | 주의:{d['se']}")
                    blk.append("--- 항진균제 ---")
                    for d in COMMON_ANTIFUNGALS:
                        blk.append(f"{d['name']} | 작용:{d['moa']} | 주의:{d['se']}")
        ...
        with sel2:
            pick_abx = st.multiselect(
                "항생제(공통 포함)", [d["name"] for d in rec["항생제"]] + [d["name"] for d in COMMON_ABX], default=[]
            )
        with sel3:
        ...
        with st.expander("펼쳐서 입력하기", expanded=False):
            ...
            with colA:
                ...
                bunq = st.text_input("BUN (mg/dL) - 특수 입력란")
        ...
        if st.button("🧠 해석하기", use_container_width=True):
            ...
            if diet_list or anc_safety:
                ...
                if diet_list:
                    screen_text += ["좋은 예시: " + ", ".join(diet_list)]
            ...
            if id_ok:
                if "registered_ids" not in st.session_state:
                    st.session_state.registered_ids = set()
                ...
                for _, k in LAB_ORDER:
                    row[k] = None if labs.get(k) is None else float(labs.get(k))
                st.session_state.history.setdefault(user_key, []).append(row)
                st.success("결과가 저장되었습니다. (별명+PIN 기준)")

                # 그래프
                if PANDAS_AVAILABLE:
                    data = st.session_state.history.get(user_key, [])
                    if data:
                    ...
                    st.info("pandas가 없어 그래프는 표시되지 않습니다.")
            else:
...
def run_cli_fallback():
    print("[CLI] Streamlit 미설치 환경 감지 → 폴백 모드로 실행합니다.")
    # 자가 테스트 실행
    try:
        _self_tests()
        print("[CLI] 내부 테스트 통과 ✅")
    except Exception as e:
        print(f"[CLI] 내부 테스트 실패: {e}")

    # 샘플 입력으로 보고서 생성
    labs = {"WBC": 3.0, "Hb": 11.2, "PLT": 120, "ANC": 800, "Na": 134, "K": 3.3, "Albumin": 3.2}
    lab_lines = interpret_labs(labs)
    diet_lines = ["좋은 예시: " + ", ".join(foods_from_labs(labs))]
    anc_lines = anc_food_safety(labs.get("ANC"))
    sp_lines = interpret_special_tests({"알부민뇨": "+"}, {"C3": "60", "TG": "220", "HDL": "35", "LDL": "170", "BNP": "110"})
    md_text = make_report_md(
        header="BloodMap 보고서 (CLI 샘플)",
        labs=labs,
        lab_lines=lab_lines,
        diet_lines=diet_lines,
        anc_lines=anc_lines,
        drug_block="- 진단: **혈액암 - APL**\n  - 항암제(선택):\n    - ATRA (베사노이드) | 기전: 분화 | 부작용: 두통",
        sp_lines=sp_lines,
    )
    ...
    out_md = os.path.join(out_dir, "bloodmap_report_sample.md")
    with open(out_md, "w", encoding="utf-8") as f:
        f.write(md_text)
    print(f"[CLI] 샘플 보고서를 저장했습니다: {out_md}")
    print("\n===== 미리보기 =====\n")
    print(md_text[:1200])

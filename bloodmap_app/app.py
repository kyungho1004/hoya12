...
def main():
    ...
    with c1:
        st.link_button("📱 카카오톡/메신저", "https://hoya12-apbyuxu4tcetrccenk8ewa.streamlit.app/")
    with c2:
        st.link_button("📝 카페/블로그", "https://cafe.naver.com/bloodmap")
    with c3:
        st.code("https://hdzwo5ginueir7hknzzfg4.streamlit.app/", language="text")

    st.caption("✅ 모바일 줄꼬임 방지 · 별명+PIN 저장/그래프 · 암별/소아/희귀암 패널 · PDF 한글 폰트 고정 · 수치 변화 비교 · 항암 스케줄표 · ANC 가이드 · 특수검사(보체/단백뇨/혈뇨/요당)")
    ...
    with c2:
        pin = st.text_input("PIN(4자리)", placeholder="0000", max_chars=4, type="password")
    with c3:
    ...
    if nickname and pin and pin.isdigit() and len(pin) == 4:
        profile_key = f"{nickname}#{pin}"
        st.session_state.last_profile_key = profile_key
    elif nickname or pin:
        st.warning("PIN은 숫자 4자리여야 합니다. (예: 0312)")
    ...
    if mode == "일반/암":
        ...
        if group == "혈액암":
            ...
            if cancer == "육종(Sarcoma)":
                sarcoma_sub = st.selectbox("육종 세부 진단", ["연부조직육종(일반)", "골육종(Osteosarcoma)", "유잉육종(Ewing)", "평활근육종(Leiomyosarcoma)", "횡문근육종(Rhabdomyosarcoma)", "지방육종(Liposarcoma)", "MPNST"])
        elif group == "소아암":
    ...
    if mode == "일반/암" and group and group != "미선택/일반" and cancer:
        ...
        # 육종 세부 진단별 기본 조합(요약)
        sarcoma_defaults = {
            "연부조직육종(일반)": ["Doxorubicin","Ifosfamide","Pazopanib"],
            "골육종(Osteosarcoma)": ["MTX","Doxorubicin","Cisplatin"],
            "유잉육종(Ewing)": ["Vincristine","Doxorubicin","Cyclophosphamide","Ifosfamide","Etoposide"],
            "평활근육종(Leiomyosarcoma)": ["Doxorubicin","Ifosfamide","Pazopanib"],
            "횡문근육종(Rhabdomyosarcoma)": ["Vincristine","Actinomycin D","Cyclophosphamide"],
            "지방육종(Liposarcoma)": ["Doxorubicin","Ifosfamide"],
            "MPNST": ["Doxorubicin","Ifosfamide"]
        }

        default_drugs_by_group = {
            "혈액암": heme_by_cancer.get(cancer, []),
            "고형암": (sarcoma_defaults.get(sarcoma_sub, solid_by_cancer.get(cancer, [])) if cancer=="육종(Sarcoma)" else solid_by_cancer.get(cancer, [])),
    ...
    if mode == "일반/암":
        if st.checkbox("⚙️ PC용 표 모드(가로형)", key="tbl", help="모바일은 세로형 고정 → 줄꼬임 없음."):
            render_inputs_table()
        else:
    ...
    # ===== 특수검사(보체/요검) =====
    st.divider()
    st.header("3️⃣ 특수검사 (선택 입력)")
    with st.expander("보체/소변 검사 열기", expanded=False):
        spec = {}
        spec["C3"] = num_input_generic("보체 C3 (mg/dL)", key="sp_c3", decimals=0, placeholder="예: 85")
        spec["C4"] = num_input_generic("보체 C4 (mg/dL)", key="sp_c4", decimals=0, placeholder="예: 8")
        spec["Proteinuria"] = num_input_generic("단백뇨(정성: 0/1)", key="sp_ProtU", decimals=0, placeholder="0 또는 1")
        spec["Hematuria"]   = num_input_generic("혈뇨(정성: 0/1)", key="sp_HemeU", decimals=0, placeholder="0 또는 1")
        spec["Glycosuria"]  = num_input_generic("요당(정성: 0/1)", key="sp_GluU", decimals=0, placeholder="0 또는 1")
        extras["special"] = spec
    ...
    if show_detail:
        ...
        def _is_qualitative(label: str) -> bool:
            l = (label or "").lower()
            return ("(0/1)" in l) and ("score" not in l and "점수" not in l)

        for key, label, unit, decs in items:
            if _is_qualitative(label):
                choice = st.selectbox(
                    f"{label}".replace("(0/1)", "").strip() + " (정성)",
                    ["음성(-)", "양성(+)"],
                    key=f"extra_{key}"
                )
                val = 1 if choice == "양성(+)" else 0
            else:
                ph = f"예: {('0' if decs==0 else '0.'+('0'*decs))}" if decs is not None else ""
                val = num_input_generic(
    ...
    # ===== Schedule =====
    render_schedule(profile_key or nickname or "")
    ...
    if run:
        ...
        if mode == "일반/암":
            ...
            if profile_key and "records" in st.session_state and st.session_state.records.get(profile_key):
                st.markdown("### 🔍 수치 변화 비교 (이전 기록 대비)")
                cmp_lines = compare_with_previous(profile_key, {k: vals.get(k) for k in ORDER if entered(vals.get(k))})
                if cmp_lines:
        ...
        meta = {
            ...
            "ped_topic": ped_topic, "sarcoma_sub": sarcoma_sub,
        }

        meds_lines = summarize_meds(meds) if meds else []
        abx_lines = abx_summary(extras.get("abx", {})) if extras.get("abx") else []
        cmp_lines = compare_with_previous(profile_key, {k: vals.get(k) for k in ORDER if entered(vals.get(k))}) if (mode=="일반/암") and profile_key else []
        ...
        try:
            ...
            st.info("PDF 생성 시 한글 폰트 적용(없으면 기본 폰트로 대체)")
        ...
        if profile_key:
            ...
            st.session_state.records.setdefault(profile_key, []).append(rec)
            st.success("저장되었습니다. 아래 그래프에서 추이를 확인하세요.")
        else:
            st.info("별명과 PIN(4자리)을 입력하면 추이 그래프/스케줄 저장이 가능합니다.")

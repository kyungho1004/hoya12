...
# === 👶 소아 증상 입력들 위/아래 어느 위치든 함께 사용 가능 ===
def render_caregiver_notes_peds(*, stool, fever, persistent_vomit, oliguria,
    ...
    if stool in ["3~4회","5~6회","7회 이상"]:
        st.markdown("""
### 💡 보호자님께 드리는 설명: 설사/장염 의심
아이가 하루 **3번 이상 묽은 변**을 보면 장염 가능성이 있어요.
특히 변 색이 **노란색/초록색**, **거품 많음**, **냄새 심함**일 땐 **로타·노로바이러스**를 고려합니다.

▶ **수분 보충은 필수!**  
- **전해질 음료(ORS)**, **미음/쌀죽**, 물 위주  
- 심한 설사/탈수 시 병원에서 **수액 치료**가 필요할 수 있어요.

**바로 병원 가야 하는 경우:** 변에 피가 섞임, 고열 동반, 심한 탈수(소변 거의 없음/축 늘어짐).
""")
    ...
    if fever in ["38~38.5","38.5~39","39 이상"]:
        st.markdown("""
### 🌡️ 발열 대처
- 옷은 가볍게, 실내는 시원하게(과도한 땀내기 금지)  
- **미온수 마사지**는 단기간 시도만  
- **해열제 간격**: 아세트아미노펜 ≥4시간, 이부프로펜 ≥6시간  
- 기저 질환/영아(특히 3개월 미만)에서 고열이면 **진료 권장**
""")
    ...
    if persistent_vomit:
        st.markdown("""
### 🤢 구토가 지속될 때
- 10~15분마다 소량씩 **수분 보충**(ORS/미지근한 물)  
- 우유/기름진 음식은 일시적으로 피하기  
- 6시간 이상 물도 못 마시거나, **초록색/커피색 토물**, **혈토**가 보이면 즉시 병원
""")
    ...
    if oliguria:
        st.markdown("""
### 🚨 탈수 의심(소변량 급감)
- 입술 마름, 눈물 없음, 피부 탄력 저하, 축 늘어짐이 동반되면 **중등~중증 탈수** 가능  
- **ORS**로 빠르게 보충하고, **지속되면 진료**가 필요합니다.
""")

    # 상기도감염/기침·콧물
    if cough in ["보통","심함"] or nasal in ["진득","누런"]:
        st.markdown("""
### 🤧 코감기/상기도 감염 관리
- 콧물은 **생리식염수/흡인기**로 부드럽게 제거  
- 수면 시 **머리 높이기**, 실내 습도 유지  
- 기침이 심해 **숨차함/청색증**/가슴함몰이 보이면 즉시 병원
""")
...
- **양쪽 눈**, **고열 동반**, **눈 통증/빛 통증** 있으면 진료 권장
""")
    ...
    if abd_pain:
        st.markdown("""
### 😣 복통/배 마사지 거부
- 오른쪽 아랫배 통증, 보행 시 통증 심해짐, 구토/발열 동반 시 **충수염** 등 평가 필요  
- 혈변/흑색변 동반 시 즉시 병원
""")
    ...
    if ear_pain:
        st.markdown("""
### 👂 귀 통증/만지면 울음 (중이염 의심)
- 누우면 통증 심해질 수 있어 **머리 살짝 높이기**  
- 38.5℃ 이상 고열, 지속 통증, **귀 분비물**이 보이면 진료 필요
""")
    ...
    if rash:
        st.markdown("""
### 🩹 발진/두드러기
- 가려움 동반 시 **미온 샤워**, 면 소재 옷
- **호흡곤란/입술 부종/전신 두드러기**가 나타나면 **알레르기 반응** 가능 → 즉시 병원
""")
with t_peds:
    ...
    with e3: rash     = st.checkbox("발진/두드러기", key=wkey("p_rash"))
with t_peds:
    st.subheader("소아 증상 기반 점수 + 해열제 계산")
    c1,c2,c3,c4,c5 = st.columns(5)
    with c1: nasal = st.selectbox("콧물", ["없음","투명","진득","누런"], key=wkey("p_nasal"))

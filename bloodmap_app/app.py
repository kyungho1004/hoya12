...
st.set_page_config(page_title="피수치 홈페이지 - 완전 적용본", layout="wide")
...
# ---------- Layout: Tabs ----------
t_home, t_peds, t_report, t_graph = st.tabs(["홈","소아","보고서","그래프"])

with t_home:
    st.markdown("### 피수치 홈페이지(데모)")
    st.caption("소아 증상 + 보호자 설명 + 보고서 + 그래프 탭 분리 완전 적용본 예시")
...
with t_peds:
    ...
    with r1b:
        wheeze = st.selectbox("쌕액거림(천명)", ["없음","조금","보통","심함"], key=wkey("p_wheeze"))

    st.divider()
    colA, colB = st.columns([2,1])
    with colA:
        dur = st.selectbox("증상 지속일수", ["선택 안 함","<1일","1-3일","4-7일",">7일"], key=wkey("p_dur"))
        max_temp = st.number_input("최고 체온(℃)", value=34.0, step=0.1, key=wkey("p_maxtemp"))
    with colB:
    ...
    score["호흡기 주의"] += map4.get(nasal,0) + map4.get(cough,0)
    score["소화기 주의"] += map4.get(stool,0)
    score["호흡기 주의"] += map4.get(eye,0)
    # 가래/천명 가중치
...
with t_report:
    ...
    md = "\n".join(lines)
    st.markdown(md)
    st.download_button("보고서(.md) 다운로드", md.encode("utf-8"), file_name="report_peds.md")

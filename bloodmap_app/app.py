...
def render_caregiver_notes_peds(
    *,
    ...
    # auto-detect ANC<500 if not explicitly provided
    if anc_low is None:
        try:
...
- **ORS 섭취**: 처음 1시간 **10–20 mL/kg**(5~10분마다 소량), 이후 설사 1회당 **5–10 mL/kg**
...
- 과일은 **껍질 제거** 후 섭취, 프룬/배 주스는 **끓여 식힌 물 1:1 희석**
    ...
    if fever in ["38~38.5", "38.5~39", "39 이상"]:
        bullet(
            "🌡️ 발열 대처",
            """
- 옷은 가볍게, 실내를 시원하게(과도한 땀내기는 X)
- **해열제 간격**: 아세트아미노펜 ≥4h, 이부프로펜 ≥6h (총량 초과 금지)
- **연락 기준(KST)**: **≥38.5℃**면 병원/주치의 연락
- **내원 기준(KST)**: **≥39.0℃** 또는 무기력/경련/탈수/호흡곤란 동반 시 **즉시 병원**
            """,
        )
    ...
    if persistent_vomit:
        bullet(
            "🤢 구토 지속",
            """
- 10~15분마다 **소량씩 수분**(ORS/미지근한 물), **우유·기름진 음식 일시 회피**
...
if ae_map:
    import re as _re
    cat_map = {
        "혈액": r"골수억제|호중구감소|빈혈|혈소판|출혈",
        "간": r"간손상|AST|ALT|빌리루빈|간독성",
        "신장": r"신독성|크레아티닌|Cr|eGFR|단백뇨",
        "심장": r"심부전|심근|QT|부정맥|흉통",
        "폐/호흡": r"폐렴|간질성폐질환|호흡곤란|저산소",
        "신경": r"말초신경|저림|감각저하|소뇌|경련|혼동",
        "위장관": r"구내염|오심|구토|설사|복통|변비",
        "피부/주사부위": r"발진|가려움|광과민|외유출|주사부위",
        "내분비/대사": r"고혈당|지질|칼륨|마그네슘|칼슘",
    }
    red_flag = _re.compile(r"아나필락시스|심부전|폐렴|저산소|중증|의식저하|소뇌|출혈|혈전|췌장염|신부전|황달|QT", _re.I)
    for k, arr in ae_map.items():
        st.write(f"- **{display_label(k, DRUG_DB)}**")
        for ln in arr:
            st.write(f"  - {ln}")
        with st.expander("분류 요약/핵심 경고"):
            for cat, pat in cat_map.items():
                m = [x for x in arr if _re.search(pat, x)]
                if m:
                    st.write(f"  • {cat}: " + "; ".join(m[:6]))
            rf = [x for x in arr if red_flag.search(x)]
            if rf:
                st.error("  🚨 핵심 경고: " + "; ".join(rf[:6]))
        # 모니터링 제안(태그/텍스트 기반)
        meta = _meta_for_drug(k)
        mon = []
        tags = meta.get("tags", set())
        if "myelosuppression" in tags:
            mon.append("CBC(ANC/PLT/Hb)")
        if "qt_prolong" in tags:
            mon.append("EKG·전해질(K/Mg/Ca)")
        if any(s in tags for s in ["renal","kidney","nephro"]) or any(t for t in arr if ("신" in t and "독" in t) or "Cr" in t or "eGFR" in t):
            mon.append("Cr/eGFR·소변(단백/혈뇨)")
        if any(u for u in arr if ("간" in u) or ("AST" in u) or ("ALT" in u) or ("빌리루빈" in u)):
            mon.append("AST/ALT/T.bil")
        if any("폐" in v or "호흡" in v for v in arr):
            mon.append("호흡증상·SpO₂")
        if any("청력" in v or "이명" in v for v in arr):
            mon.append("청력검사(필요 시)")
        if mon:
            st.info("🧪 모니터링 제안: " + " · ".join(sorted(set(mon))))
else:
...
with t_peds:
  ...
  render_caregiver_notes_peds(
    ...
    migraine=migraine,
    hfmd=hfmd,
    constipation=st.checkbox("변비(몇일째, 덩어리딱딱함 등)", key=wkey("p_constipation"))
)

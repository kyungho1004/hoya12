...
def _mount_tab_autoselect(selected_key:str):
    """
    Keep visual tab order fixed but auto-select target tab using a tiny JS click.
    selected_key: one of {"home","labs","dx","chemo","peds","special","report","graph"}
    """
    label_map = {
        "home": "🏠 홈",
        "labs": "🧪 피수치 입력",
    ...
    label = label_map.get(selected_key, label_map["home"])
    # Only inject when not already the first tab (reduces flicker)
...
def _mount_tab_autoselect(selected_key: str):
    ...
    function clickTab(){{
        const tabs = Array.from(document.querySelectorAll('[data-baseweb="tab"], button[role="tab"]'));
        if (!tabs.length) return false;
        const btn = tabs.find(el => (el.innerText || '').trim().startsWith(targetLabel));
        if (btn){{
            const ariaSelected = btn.getAttribute('aria-selected');
            if (ariaSelected !== 'true') btn.click();
            return true;
        }}
        return false;
    }}
    (function(){{
        if (!clickTab()) {{
            const obs = new MutationObserver((m)=>{{ if (clickTab()) obs.disconnect(); }});
...
order = tab_labels
_tabs = st.tabs(order)
_mount_tab_autoselect(_selected)

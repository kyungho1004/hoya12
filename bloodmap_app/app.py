...
def _render_carelog_input():
    ...
    with col3:
        ...
        if dt_widget:
            ts = dt_widget("복용 시각", value=datetime.now(_KST), key="cl_ts_dt")
        else:
            d = st.date_input("복용 날짜", value=datetime.now(_KST).date(), key="cl_dt_d")
            t = st.time_input("복용 시간", value=datetime.now(_KST).time(), key="cl_dt_t")
        ...
        if st.button("오늘 기록 초기화", key="cl_reset_today"):
            ...
            for r in st.session_state["care_log"]:
                try:
                    tsr = r.get("ts","")[:19]
                    from datetime import datetime as _dt
                    d0 = _dt.fromisoformat(tsr).date()
                except Exception:
                    d0 = today  # 파싱 실패 시 오늘로 간주하여 삭제
                if d0 != today:
                    newlog.append(r)

...
# ===== BloodMap mellow route guard + state hydration (patch) =====
def _bm_defaults():
    ss = st.session_state
    # Route
    ss.setdefault("_route", ss.get("_route_last", "dx"))
    ss.setdefault("_route_last", ss.get("_route", "dx"))
    ss.setdefault("_home_intent", True)  # allow home unless explicitly blocked
    ss.setdefault("_ctx_tab", ss.get("_ctx_tab", None))
    # Critical dicts — never drop user inputs on rerun
...
def _bm_hardlock():
    ...
    if cur == "home" and not ss.get("_home_intent", True):
        # do not rerun; just stick to last route to avoid flushing inputs
        ss["_route"] = ss.get("_route_last", "dx")
def _pin_route(name: str):
    ss = st.session_state
    ss["_route"] = name
    if name != "home":
        ss["_route_last"] = name
    # best-effort sync query param (no rerun)
    try:
        qp = st.query_params
...
with t_special:
    # Patch: pin special context and preserve route; do not call UI here

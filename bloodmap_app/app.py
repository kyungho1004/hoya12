...
# 1) 저장 디렉터리 3단계 폴백: /mnt/data → /tmp → (메모리 세션)
paths = ["/mnt/data/metrics", "/tmp/metrics"]
root = None
for p in paths:
    try:
        os.makedirs(p, exist_ok=True)
        testfile = os.path.join(p, ".touch")
...
# 2) 파일 경로 결정 (파일 불가하면 세션 메모리 사용)
usage = {"total": 0, "by_date": {}}
path = None
if root:
    ...
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                usage = json.load(f) or usage
        except Exception:
            pass
else:
    # 파일 저장 불가 환경 → 세션 메모리 이용
    if "_usage_mem" in st.session_state:
        usage = st.session_state["_usage_mem"]

# 3) 세션당/하루 1회 증가 시도 (파일 실패 시에도 메모리에 반영)
key = f"_usage_counted_{today}"
if not st.session_state.get(key, False):
    usage["total"] = int(usage.get("total", 0)) + 1
    by = usage.get("by_date", {})
    by[today] = int(by.get(today, 0)) + 1
    usage["by_date"] = by
    ...
    if path:
        ...
        st.session_state["_usage_mem"] = usage

# 4) 항상 뱃지는 렌더링 (집계 실패해도 0으로 표시)
today_count = int(usage.get("by_date", {}).get(today, 0))
total_count = int(usage.get("total", 0))

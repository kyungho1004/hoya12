# app.py — Bloodmap SAFE CLASSIC + SpecialTests Robust Loader (2025-11-02 KST)
# - 홈 튐 방지(_route 고정)
# - 특수검사(special_tests.py) 강제 경로 탐색 + 동적 로드(없으면 더미 UI)
# - 과거 monkey-patch 원복(재귀/중첩 차단)
# - 위젯 키 네임스페이스로 중복 방지
# - /mnt/data → /mount/data → /tmp 경로 폴백(그래프/피드백 저장)
# - 응급 배너/ANC 식이가이드 요약/소아 해열제 간격 가드(APAP≥4h, IBU≥6h)
# - 외부 모듈(onco_map, drug_db, pdf_export) 있으면 사용, 없으면 스텁

from __future__ import annotations
import os, re, sys, io, json, csv, importlib.util
...
# 외부에서 st.text_input 등을 monkey patch 했던 흔적이 있을 때 원복
...
def _set_qp(**kwargs):
    try:
        st.query_params.update(**kwargs)  # 최신 API
    except Exception:
        try:
...
def _bootstrap_route():
    ...
    if not cur:
        if url_r:
            ...
            ss["_route"] = "chemo"   # 초기 진입 고정(형 기존 흐름)
...
# 1) PAGE SHELL
# =========================
APP_VERSION = "Bloodmap SAFE Classic • 2025-11-02"
...
> Made with 💜 so that caregivers can feel a little safer each day."""
)
st.markdown("---")

def render_deploy_banner(url: str):
    st.markdown(
        f'<div style="padding:.5rem 0;color:#666;">'
        f'<strong>배포</strong>: <a href="{url}" target="_blank">{url}</a><br>'
        f'제작: Hoya/GPT · 자문: Hoya/GPT · KST 기준'
        f'</div>',
        unsafe_allow_html=True
    )
render_deploy_banner("https://bloodmap.streamlit.app/")
...
# =========================
# 4) OPTIONAL MODULES (safe import → stub)
# =========================
# onco_map
try:
    from onco_map import build_onco_map, dx_display  # type: ignore
except Exception:
    ...
    def dx_display(group: str, disease: str) -> str:
        DX_KO = {"ALL":"급성 림프구성 백혈병","AML":"급성 골수성 백혈병",
                 "Lymphoma":"림프종","Breast":"유방암","Colon":"대장암","Lung":"폐암"}
        ko = DX_KO.get(disease) or disease
...
# drug_db
try:
    from drug_db import DRUG_DB  # type: ignore
except Exception:
...
# =========================
# 4.5) SPECIAL TESTS ROBUST LOADER (NEW)
# =========================
def _load_special_tests_ui_callable():
    """
    special_tests.py를 다양한 위치에서 탐색해서 동적으로 로드.
    찾으면 special_tests.special_tests_ui (callable) 반환, 없으면 None.
    """
    # 이미 import 되어있으면 그대로 사용
    try:
        import special_tests as _st_mod  # noqa
        mod0 = sys.modules.get('special_tests')
        fn = None
        if mod0 is not None:
            for attr in ('special_tests_ui','render','ui'):
                cand = getattr(mod0, attr, None)
                if callable(cand):
                    fn = cand
                    break
        if callable(fn):
    ...
    search_dirs = [
        base,                       # 루트(= app.py와 같은 폴더)
        base / "utils",
        base / "quarantine",
        base / "data",
        Path.cwd(),                 # 현재 작업 디렉토리
        Path('/mnt/data'),          # 고정 외부 경로(업로드 위치)
    ]
    candidates = []
    for d in search_dirs:
        for name in ("special_tests.py", "special_tests/__init__.py"):
...
def _render_special_tests_safe():
    ...
    특수검사 탭에서 호출할 안전 엔트리.
    - 찾으면 실제 UI를 실행
    - 못 찾으면 왜 못 찾았는지 경로 디버그 출력 + 더미 UI
    """
    fn = _load_special_tests_ui_callable()
    if callable(fn):
        try:
            fn()  # 정상 모듈 UI 실행
            return
        except Exception as e:
            st.error(f"특수검사 모듈 실행 중 오류가 발생했지만 앱은 계속 동작합니다: {e}")

    # 못 찾았을 때 — 디버그 정보 + 더미
    st.warning("special_tests.py를 아직 찾지 못했습니다. 아래 탐색 경로를 확인하세요.")
    base = Path(__file__).parent
    search_dirs = [base, base / "utils", base / "quarantine", base / "data", Path.cwd(), Path('/mnt/data')]
    st.caption("탐색 디렉토리:")
    for d in search_dirs:
        st.write(f"• {d}")

    st.info("임시 UI(더미)를 표시합니다. special_tests.py를 위 경로들 중 한 곳에 저장하면 자동으로 전환됩니다.")
    col = st.columns(3)
    with col[0]: st.checkbox("Albumin(뇨)+", key=wkey("sp_alb"))
    with col[1]: st.checkbox("WBC(뇨)+", key=wkey("sp_wbc"))
    with col[2]: st.checkbox("Nitrite+", key=wkey("sp_nit"))
...
def emergency_level(labs: dict, temp_c, hr, symptoms: dict):
    def add(reasons, contrib, name, base, wkey):
        W = get_weights(); w = W.get(wkey,1.0); s=base*w
    ...
    heart = _try_float(hr)
    reasons, contrib = [], []
    if a is not None and a < 500:   add(reasons,contrib,"ANC<500",3,"w_anc_lt500")
    elif a is not None and a < 1000:add(reasons,contrib,"ANC 500~999",2,"w_anc_500_999")
    if t is not None and t >= 38.5: add(reasons,contrib,"고열 ≥38.5℃",2,"w_temp_ge_38_5")
    elif t is not None and t >= 38: add(reasons,contrib,"발열 38.0~38.4℃",1,"w_temp_38_0_38_4")
    if p is not None and p < 20000: add(reasons,contrib,"혈소판 <20k",2,"w_plt_lt20k")
    if h is not None and h < 7.0:   add(reasons,contrib,"중증 빈혈(Hb<7)",1,"w_hb_lt7")
    if c is not None and c >= 10:   add(reasons,contrib,"CRP ≥10",1,"w_crp_ge10")
    if heart and heart > 130:       add(reasons,contrib,"빈맥(HR>130)",1,"w_hr_gt130")
    for k,wk,base in [
        ...
        if symptoms.get(k): add(reasons,contrib,k,base,wk)
...
with t_home:
    ...
    with st.expander("💬 피드백(앱 개선 제안/오류 신고)", expanded=False):
        ...
        _tags = st.multiselect("빠른 태그", ["속도 빨라요","설명 명확","UI 편해요","오류 있음","모바일 불편","기능 부족","응급도 정확"],
...
# ---------- PEDS ----------
with t_peds:
    st.subheader("소아 증상 + 보호자 설명 + 해열제 계산")
    ...
    with c4: fever = st.selectbox("발열", ["없음","37~37.5","37.5~38","38~38.5","38.5~39","39 이상"], key=wkey("p_fever"))
    with c5: eye   = st.selectbox("눈곱/결막", ["없음","맑음","노랑-농성","양쪽"], key=wkey("p_eye"))

    g1,g2 = st.columns(2)
    with g1: sputum = st.selectbox("가래", ["없음","조금","보통","많음"], key=wkey("p_sputum"))
    with g2: wheeze = st.selectbox("쌕쌕(천명)", ["없음","조금","보통","심함"], key=wkey("p_wheeze"))
    d1,d2,d3 = st.columns(3)
    with d1: oliguria = st.checkbox("소변량 급감", key=wkey("p_oliguria"))
    with d2: persistent_vomit = st.checkbox("지속 구토(>6h)", key=wkey("p_pvomit"))
    ...
    danger = 0
    if max_temp and max_temp >= 38.5: danger += 1
    if petechiae: danger += 1
    if persistent_vomit and oliguria: danger += 1
    if wheeze in ["보통","심함"]: danger += 1

    if max_temp and max_temp >= 39.0 or petechiae:
        st.error("🚨 고위험 신호 — 즉시 병원 권장")
    elif danger >= 2:
        st.warning("🟡 주의 — 면밀 관찰·수분 보충")
    else:
        st.info("🟢 비교적 안정")
    ...
    if anc_val is not None and anc_val < 1000:
        st.warning("🍽️ 저호중구: 생야채/껍질과일 피하고 완전 가열, 남은 음식 2시간 이후 섭취 비권장, 멸균·살균 식품 권장.")

    st.markdown("---")
    st.subheader("해열제 계산기(간이)")
    wt = st.number_input("체중(kg)", min_value=0.0, max_value=200.0,
                         value=_safe_float(st.session_state.get(wkey("wt_peds"),0.0),0.0),
                         step=0.1, key=wkey("wt_peds_num"))
    st.session_state[wkey("wt_peds")] = wt
    apap_mg = max(0.0, 12.5 * wt)              # APAP 10~15 mg/kg → 12.5 기준
    apap_ml = apap_mg/160*5 if wt>0 else 0.0   # 160mg/5mL 시럽
    ibu_mg  = max(0.0, 10.0 * wt)              # IBU 10 mg/kg
    ibu_ml  = ibu_mg/100*5 if wt>0 else 0.0    # 100mg/5mL 시럽
    colA,colB = st.columns(2)
    with colA: st.write(f"아세트아미노펜(160mg/5mL): **{apap_ml:.1f} mL** — 간격 **≥4h**")
    with colB: st.write(f"이부프로펜(100mg/5mL): **{ibu_ml:.1f} mL** — 간격 **≥6h**")
    st.caption("※ 금기/주의 질환은 반드시 의료진 지시 우선.")

# ---------- DX ----------
with t_dx:
    st.subheader("암 선택")
    def build_onco_map_stub():
        return {
            "혈액암": {"ALL": {}, "AML": {}, "Lymphoma": {}},
            "고형암": {"Breast": {}, "Colon": {}, "Lung": {}},
        }
    ONCO = (build_onco_map() if 'build_onco_map' in globals() else build_onco_map_stub()) or {}
...
with t_special:
    st.subheader("🔬 특수검사")
    _render_special_tests_safe()   # ← 강제 로더 사용
...
with t_report:
    ...
    md = "\n".join(lines)
    # 특수검사 md가 있으면 합치기
    sp_md = st.session_state.get("special_tests_report_md")
    if sp_md:
        md += "\n\n" + sp_md

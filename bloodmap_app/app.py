...
def main():
    ...
    # Hide +/- buttons on number_input and spinner arrows
    st.markdown(
        '''<style>
    ...
    if t_cancer:
        ...
        if grp == "혈액암":
            dx, dx_label = st.selectbox("진단명", HEMATO_OPTS, format_func=lambda x: x[1])
        elif grp == "육종(진단명 분리)":
            dx, dx_label = st.selectbox("육종 세부", SARCOMA_OPTS, format_func=lambda x: x[1])
        else:
            dx, dx_label = st.selectbox("고형암", SOLID_OPTS, format_func=lambda x: x[1])
    else:
        dx = None
    ...
    if t_peds_daily or t_peds_inf:
        ...
        st.caption("암종에 맞는 항암제 목록을 보여드리지만 자동 선택은 하지 않습니다. 직접 선택하세요.")
        sel_chemo = st.multiselect("항암제(한글 병기)", default_chemo + CHEMO_COMMON, default=[], key="chemo_sel")
        sel_abx = st.multiselect("항생제(한글 병기)", ANTIBIOTICS_COMMON, default=[], key="abx2_sel")
    ...
    if go:
        ...
        for d in (sel_chemo or []):
            ...
            if "ARA-C" in d or "Cytarabine" in d:
                warn += ["[ARA-C] 고용량 시 소뇌실조/결막염 — 점안 스테로이드 고려, 신경증상 즉시 보고"]
            ...
            if "Fludarabine" in d:
                warn += ["[Fludarabine] 면역억제 — 기회감염 예방/백신 상담"]
            if "G-CSF" in d:

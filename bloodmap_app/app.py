...
APP_VERSION = "v7.17d (Safe Import + Peds)"
...
def _load_local_module(mod_name: str, rel_paths):
    ...
    for rel in rel_paths:
        ...
        if cand.exists():
            ...
            return mod, str(cand)
    # also try normal import path, but do not crash if missing
    try:
...
st.caption(f"모듈 경로 — special_tests: {SPECIAL_PATH or '(not found)'} | peds_dose: {PD_PATH or '(not found)'}")
...
def emergency_level(labs: dict, temp_c, hr, symptoms: dict):
    ...
    W = get_weights()
    reasons = []
    contrib = []  # list of dicts: {"factor":..., "base":x, "weight":w, "score":base*w}
...
# ====== Tabs ======
tab_labels = ["🏠 홈","🧪 피수치 입력","👶 소아 증상","🔬 특수검사","📄 보고서"]
t_home, t_labs, t_peds, t_special, t_report = st.tabs(tab_labels)
...
# ====== PEDS (symptom-based + antipyretic dosing) ======
with t_peds:

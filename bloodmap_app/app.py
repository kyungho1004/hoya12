...
# === PEDS jumpbar (id-based, no reroute) ===
...
# --- backward-compat shim: legacy paddles -> jumpbar ---
...
# === PEDS focus (native, no JS) ===
def _peds_set_focus(sec: str):
    try:
        import streamlit as st
        st.session_state[wkey("peds_focus")] = str(sec)
    except Exception:
        pass

def _peds_is_focus(sec: str) -> bool:
    try:
        import streamlit as st
        return st.session_state.get(wkey("peds_focus")) == str(sec)
    except Exception:
        return False
# === /PEDS focus ===


# === PEDS native jumpbar (session_state based) ===
def render_peds_jumpbar_native():
    import streamlit as st
    st.markdown('<div class="peds-jumpbar" />', unsafe_allow_html=True)
    c1,c2,c3 = st.columns(3)
    with c1:
        st.button("🧻 변비", key=wkey("peds_go_const"), use_container_width=True,
                  on_click=_peds_set_focus, args=("constipation",))
    with c2:
        st.button("💦 설사", key=wkey("peds_go_diarr"), use_container_width=True,
                  on_click=_peds_set_focus, args=("diarrhea",))
    with c3:
        st.button("🤢 구토", key=wkey("peds_go_vomit"), use_container_width=True,
                  on_click=_peds_set_focus, args=("vomit",))
    c4,c5,c6 = st.columns(3)
    with c4:
        st.button("🌡️ 해열제", key=wkey("peds_go_fever"), use_container_width=True,
                  on_click=_peds_set_focus, args=("antipyretic",))
    with c5:
        st.button("🥤 ORS·탈수", key=wkey("peds_go_ors"), use_container_width=True,
                  on_click=_peds_set_focus, args=("ors",))
    with c6:
        st.button("🫁 가래·쌕쌕", key=wkey("peds_go_respi"), use_container_width=True,
                  on_click=_peds_set_focus, args=("resp",))
# === /PEDS native jumpbar ===
...
with t_labs:
    ...
    st.markdown("""
<script>
try{ localStorage.setItem('__active_tab_label__','피수치'); }catch(e){}
</script>
<!-- labs-peds-sticky -->
""", unsafe_allow_html=True)

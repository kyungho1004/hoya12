...
# --- Quick Actions: 변비/설사/구토 바로가기 (peds_guide) ---
try:
    from peds_guide import (
        render_section_constipation as _pg_constip,
        render_section_diarrhea as _pg_diarr,
        render_section_vomit as _pg_vomit,
    )
    _has_peds_triple = True
except Exception:
    _has_peds_triple = False

col_quick = st.columns([1,1,1,3])
with col_quick[0]:
    if st.button("🚽 변비", key=wkey("btn_peds_constip")) and _has_peds_triple:
        try:
            _pg_constip()
        except Exception:
            st.warning("변비 섹션 렌더 중 예외가 발생했습니다.")
with col_quick[1]:
    if st.button("💧 설사", key=wkey("btn_peds_diarr")) and _has_peds_triple:
        try:
            _pg_diarr()
        except Exception:
            st.warning("설사 섹션 렌더 중 예외가 발생했습니다.")
with col_quick[2]:
    if st.button("🤢 구토", key=wkey("btn_peds_vomit")) and _has_peds_triple:
        try:
            _pg_vomit()
        except Exception:
            st.warning("구토 섹션 렌더 중 예외가 발생했습니다.")
if not _has_peds_triple:
    st.caption("peds_guide의 변비/설사/구토 섹션을 찾을 수 없습니다. peds_guide.py 위치/함수명을 확인하세요.")
# --- End Quick Actions ---

...
# ▼ 예: 존재한다면 사용, 없으면 False 처리
infant_under_3m = globals().get("infant_under_3m", False)      # 만 3개월 미만 bool
stridor = globals().get("stridor", False)                      # 흡기성 그르렁/쌕
wheeze = globals().get("wheeze", False)                        # 천식성 쌕쌕
dysuria = globals().get("dysuria", False)                      # 배뇨통/빈뇨
constipation = globals().get("constipation", False)            # 변비 의심
head_trauma = globals().get("head_trauma", False)              # 최근 머리 다침
nonblanching_rash = globals().get("nonblanching_rash", False)  # 눌러도 안 사라지는 반점(자반)
...
if stool in ["3~4회", "5~6회", "7회 이상"]:
    bullet(
        "💧 설사/장염 의심",
        """
- 하루 **3회 이상 묽은 변**이면 장염 가능성이 있습니다.
- **노란/초록 변**, **거품 많고 냄새 심함** → 로타/노로바이러스 고려.
- **대처**: ORS를 **5–10 mL씩 5–10분 간격**으로 자주, 미음/쌀죽 등으로 수분·전해질 보충.
- **즉시 진료**: 피 섞인 변, **담즙색(초록) 구토**, 고열, **소변 거의 없음/축 늘어짐**.
- (목표량 참고) 경증~중등 탈수는 **4시간에 50–100 mL/kg**를 목표(상황 따라 분할 섭취).
        """,
    )
...
if fever in ["38~38.5", "38.5~39", "39 이상"]:
    bullet(
        "🌡️ 발열 대처",
        """
- **옷은 가볍게**, 과한 땀복/두꺼운 이불은 피하세요.
- 실내 **24–26 ℃** 유지. 장시간 목욕은 피하고 **미지근한 물수건 닦기**는 짧게만.
- **해열제 간격**
  - 아세트아미노펜(파라세타몰): **≥ 4시간**
  - 이부프로펜: **≥ 6시간**
- **교차 복용**은 **담당의가 허용한 경우에 한해** 고려하며, 서로 **최소 2시간 간격**을 지키고 **총 24시간 용량 초과 금지**(앱 가드레일 준수).
- **위험 신호**: 38.5 ℃ 이상 지속, 의식 저하, 호흡곤란, 경련, **특히 만 3개월 미만의 발열은 즉시 진료**.
        """,
    )

# 영아 고위험(만 3개월 미만)
if infant_under_3m and fever not in [None, "없음", ""]:
    bullet(
        "🚨 영아 발열(만 3개월 미만)",
        """
- 만 3개월 미만의 **모든 발열**은 **즉시 의료기관 평가**가 필요합니다.
        """,
    )
...
if persistent_vomit:
    bullet(
        "🤢 구토 지속",
        """
- **10–15분마다 소량씩** ORS/미지근한 물을 제공합니다(한 번에 많이 X).
- 우유·기름진 음식은 일시적으로 피하세요.
- **자세**: 토할 땐 **옆으로 눕혀 머리를 약간 측면**으로 유지해 기도 흡인을 줄입니다.
- **즉시 진료**: 6시간 이상 물도 못 마심, **초록/커피색 토물**, 혈토, **지속 복통/의식 변화**.
        """,
    )
...
if (cough in ["조금", "보통", "심함"]) or (nasal in ["진득", "누런"]):
    bullet(
        "🤧 기침·콧물(상기도감염)",
        """
- **생리식염수 세척/흡인**으로 콧물 제거, 수면 시 **머리를 살짝 높이기**.
- **즉시 진료**: 숨차함/청색증/가슴함몰/먹지 못함/탈수 소견.
        """,
    )

# 크룹(후두염) – 흡기성 그르렁/밤에 악화
if stridor:
    bullet(
        "🐶 크룹 의심(개짖는 기침/흡기성 그르렁)",
        """
- 아이를 진정시키고 **상체를 세워 호흡**을 편하게 하세요. 서늘하고 습한 공기가 도움될 수 있습니다.
- **즉시 진료**: 휴식 시에도 그르렁, **가슴함몰**, 침 흘림/삼킴 곤란, 청색증.
        """,
    )
...
if wheeze:
    bullet(
        "🫁 쌕쌕거림(천식성 호흡) 의심",
        """
- 말 끊김, 분당 호흡수 증가, **가슴함몰**이 보이면 **응급 평가**가 필요합니다.
- 흡입치료 계획을 받은 아이는 **지시된 구제 흡입제**를 우선 시행하고 **의료진과 상의**하세요.
        """,
    )
...
if eye in ["노랑-농성", "양쪽"]:
    bullet(
        "👀 결막염 의심",
        """
- **손 위생** 철저, 분비물은 깨끗한 거즈로 닦아내세요. **콘택트렌즈는 금지**.
- **바이러스성**: 양쪽 충혈·물 같은 분비물, 감기 동반 잦음.
- **세균성**: **노란 고름**, 속눈썹이 아침에 **붙는** 느낌.
- 빛 통증·시야 흐림·심한 통증 시 **안과 평가**가 필요합니다.
        """,
    )
...
if abd_pain:
    bullet(
        "😣 복통/배 마사지 거부",
        """
- 우하복부 통증·보행 악화·구토/발열 동반 → **충수염 평가 필요**.
- **담즙성(초록) 구토**, **혈변/흑변**, **심한 지속 통증** → **119/응급실** 권장.
        """,
    )
...
if dysuria:
    bullet(
        "🚻 배뇨 통증/악취 소변(요로감염 의심)",
        """
- 배뇨 시 통증, **빈뇨/급뇨**, 악취 소변, 하복부 통증/발열이 있으면 **소변검사 평가**가 필요합니다.
- 수분 섭취를 권장하되, **고열·구토·옆구리 통증** 동반 시 즉시 진료.
        """,
    )
...
if constipation:
    bullet(
        "🍐 변비 의심",
        """
- 3일 이상 딱딱한 변/통증, 배변 회피 → 수분·식이섬유·좌욕 등 완화 시도.
- **담즙성 구토, 복부 팽창, 체중 감소** 동반 시 **의료평가**.
        """,
    )
...
if ear_pain:
    bullet(
        ...
        """
- 눕기 불편하면 **머리를 살짝 높이기**.
- **38.5 ℃↑**, 통증 지속, **귀 분비물** → 진료 필요.
- 목욕 시 **귀에 물이 들어가지 않도록** 주의하세요.
        """,
    )
...
if rash:
    bullet(
        "🩹 발진/피부 자극",
        """
- **미온 샤워**, 면 소재 옷, 자극 적은 보습 로션.
- 새로운 로션/세제/바디워시 사용 여부 확인.
- **전신 심한 발진, 호흡곤란, 입술/혀 부종** → **119/응급실**.
        """,
    )

if nonblanching_rash:
    bullet(
        "🚨 눌러도 안 사라지는 붉은 반점(자반)",
        """
- 유리컵 등으로 눌러도 **사라지지 않으면** 출혈성 질환 가능 → **즉시 응급 평가**가 필요합니다.
        """,
    )
...
if hives:
    bullet(
        "⚠️ 두드러기/알레르기(주의)",
        """
- 전신 두드러기, 입술·눈 주위 부종, 구토/복통 → **알레르기 반응 가능**.
- **호흡곤란/쌕쌕/목 조임** → **즉시 응급실**.
        """,
    )
...
if migraine:
    bullet(
        "🧠 편두통 의심",
        """
- **한쪽·박동성 두통**, 빛/소리 민감, 메스꺼움 → 휴식·수분 보충.
- **번개치듯 새로 시작한 극심한 두통**, **신경학적 이상(구음장애·편마비·의식 변화)** → **응급 평가**.
        """,
    )
...
if hfmd:
    bullet(
        "✋👣 수족구 의심(HFMD)",
        """
- **손·발·입안** 물집/궤양 + 발열, 전염성 있으므로 **손 씻기/식기 분리**.
...
if head_trauma:
    bullet(
        "🪖 머리 다침(레드플래그)",
        """
- **반복 구토, 의식저하, 지속 두통, 경련, 한쪽 약함/말 어눌함** → **즉시 응급 평가**.
        """,
    )

st.info("❗ 즉시 병원 평가: 번개치는 두통 · 시야 이상/복시/암점 · 경련 · 의식저하 · 심한 목 통증 · 호흡곤란/입술부종 · 눌러도 안 사라지는 발진 · 만 3개월 미만 발열")

st.caption("※ 해열제는 **체중 기반 용량**과 **앱 가드레일**을 반드시 따르세요. 이 가이드는 참고용이며, 최종 판단은 의료진의 진료에 따릅니다.")

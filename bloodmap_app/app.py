...
# === /LOCAL MODULE LOADER v3 (robust) ===
# 패치 원칙: 삭제 금지. 기존 구조 유지하되 로더만 견고화.
def _load_special_tests_module():
    import importlib.util, sys, os, traceback
    candidates = []
    base_dir = os.path.dirname(__file__) if "__file__" in globals() else os.getcwd()
    # 탐색 후보(우선순위)
    candidates += [
        os.path.join(base_dir, "special_tests.py"),
        os.path.join(base_dir, "modules", "special_tests.py"),
    ...
    for path in candidates:
        try:
            if os.path.exists(path):
                spec = importlib.util.spec_from_file_location("special_tests", path)
    ...
    # 최후: 일반 import
    try:
        mod = __import__("special_tests")
        return mod, "<pkg-import>"
    except Exception:
        return None, last_exc or "import failed"

_sp, SPECIAL_PATH = _load_special_tests_module()
def special_tests_ui():
    import streamlit as st
    if _sp:
        # 엔트리 자동 탐색
        for attr in ("special_tests_ui", "render", "ui"):
            fn = getattr(_sp, attr, None)
            if callable(fn):
                try:
                    return fn()
                except Exception as e:
    ...
    if SPECIAL_PATH:
        st.caption(f"special_tests source: {SPECIAL_PATH}")
    return []
# === /LOCAL MODULE LOADER v3 (robust) ===

...
def _cancer_section():
    ...
    atra_caps = st.checkbox("ATRA (캡슐) 복용 중", key="atra_caps")
    # 항생제
    st.multiselect("항생제 (한글 표기)", options=config.ANTIBIOTICS, key="abx_sel")
    return group, atra_caps
...
def _pediatric_helper():
    ...
    total_months = pediatric_guard(years, months)  # 안전 가드
...
def main():
    ...
    st.divider()
    st.caption("저장 키(별명#PIN)는 중복 방지를 위해 사용됩니다. 숫자만 허용되며 자동 정리됩니다.")
    st.caption("본 앱은 보호자의 이해를 돕는 참고용입니다. 모든 의학적 판단은 의료진에게 문의하세요.")

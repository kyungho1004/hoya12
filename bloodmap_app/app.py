...
with t_peds:
    ...
    # === 변비 체크 + ANC 저하 자동 감지 ===
    constipation = st.checkbox("변비", key=wkey("p_constipation"))

    # ANC 저하 여부(세션의 검사값 기반)
    _labs = st.session_state.get("labs_dict", {}) or {}
    try:
        anc_val = float(_labs.get("ANC")) if _labs.get("ANC") not in (None, "") else None
    except Exception:
        anc_val = None

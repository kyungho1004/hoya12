...
def compute_acr(albumin_mg_L: float, urine_cr_mg_dL: float) -> float:
    if not albumin_mg_L or not urine_cr_mg_dL:
        return 0.0
    return (albumin_mg_L / urine_cr_mg_dL) * 100.0

def compute_upcr(protein_mg_dL: float, urine_cr_mg_dL: float) -> float:
    if not protein_mg_dL or not urine_cr_mg_dL:
        return 0.0
    return (protein_mg_dL / urine_cr_mg_dL) * 1000.0

def interpret_acr(acr: float) -> str:
    if acr <= 0: return "ACR: 입력값이 부족합니다."
    if acr < 30: return "ACR < 30 mg/g: 정상 범위"
    if acr <= 300: return "ACR 30~300 mg/g: 미세알부민뇨(주의)"
    return "ACR > 300 mg/g: 알부민뇨(의료진 상담 권장)"

def interpret_upcr(upcr: float) -> str:
    if upcr <= 0: return "UPCR: 입력값이 부족합니다."
    if upcr < 150: return "UPCR < 150 mg/g: 정상~경미"
    if upcr <= 500: return "UPCR 150~500 mg/g: 단백뇨(주의)"
    return "UPCR > 500 mg/g: 고단백뇨(의료진 상담 권장)"

def anc_banner(anc: float) -> str:
    if not anc: return ""
    if anc < 500: return "⚠️ 호중구(ANC) 500 미만: 외출 자제·익힌 음식·멸균 식품 권장. 조리 후 2시간 지난 음식 섭취 금지."
    if anc < 1000: return "⚠️ 호중구(ANC) 500~999: 감염 주의, 신선 채소는 세척·가열 후 섭취 권장."
...
def pediatric_guides(values: Dict[str, Any], group: str, diagnosis: str = "") -> List[str]:
    msgs: List[str] = []
    anc = float(values.get("ANC(호중구)") or 0)
    if anc:
        ab = anc_banner(anc)

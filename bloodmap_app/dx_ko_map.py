# -*- coding: utf-8 -*-
"""
DX_KO_SARCOMA: 영문 진단명을 한글 병기로 매핑 (육종 영역 보강)
- 이 파일은 드롭다운 표기(format_func) 및 진단 라벨 병기에 사용됩니다.
"""

DX_KO_SARCOMA = {
    # Common sarcomas
    "Osteosarcoma": "골육종",
    "Ewing sarcoma": "유잉육종",
    "Ewing's sarcoma": "유잉육종",
    "Leiomyosarcoma": "평활근육종",
    "Liposarcoma": "지방육종",
    "Synovial sarcoma": "윤활막육종",
    "Rhabdomyosarcoma": "횡문근육종",
    "GIST": "위장관기질종양",
    "Gastrointestinal stromal tumor": "위장관기질종양",
    "Dermatofibrosarcoma protuberans": "돌출성 피부섬유육종",
    "Angiosarcoma": "혈관육종",
    "MPNST": "악성 말초신경초종양",
    "Malignant peripheral nerve sheath tumor": "악성 말초신경초종양",
    "Chondrosarcoma": "연골육종",
    "Fibrosarcoma": "섬유육종",
    "Clear cell sarcoma": "투명세포육종",
    "Alveolar soft part sarcoma": "폐포 연조직 육종",
    "Epithelioid sarcoma": "상피양육종",
    "Myxofibrosarcoma": "점액섬유육종",
    "Undifferentiated pleomorphic sarcoma": "미분화 다형성 육종",
    "UPS": "미분화 다형성 육종",
    "Desmoid tumor": "데스모이드 종양(공격적 섬유종증)",
    "Aggressive fibromatosis": "공격적 섬유종증",
    # Soft-tissue/rare variants
    "Hemangiopericytoma": "혈관주위세포종",
    "Kaposi sarcoma": "카포시 육종",
    "Infantile fibrosarcoma": "영아 섬유육종",
    "Extraskeletal osteosarcoma": "골격외 골육종",
}


DX_KO_HEME = {
    "AML": "급성 골수성 백혈병",
    "Acute myeloid leukemia": "급성 골수성 백혈병",
    "APL": "급성 전골수구성 백혈병",
    "Acute promyelocytic leukemia": "급성 전골수구성 백혈병",
    "ALL": "급성 림프모구성 백혈병",
    "Acute lymphoblastic leukemia": "급성 림프모구성 백혈병",
    "CML": "만성 골수성 백혈병",
    "Chronic myeloid leukemia": "만성 골수성 백혈병",
    "CLL": "만성 림프구성 백혈병",
    "Chronic lymphocytic leukemia": "만성 림프구성 백혈병",

    "DLBCL": "미만성 거대 B세포 림프종",
    "Diffuse large B-cell lymphoma": "미만성 거대 B세포 림프종",
    "Hodgkin lymphoma": "호지킨 림프종",
    "Follicular lymphoma": "여포성 림프종",
    "MALT lymphoma": "MALT 림프종",
    "Mantle cell lymphoma": "외투세포 림프종",
    "Burkitt lymphoma": "버킷 림프종",
    "T-cell lymphoma": "T세포 림프종",
    "NK/T-cell lymphoma": "NK/T세포 림프종",
    "Primary CNS lymphoma": "원발성 중추신경계 림프종",
}

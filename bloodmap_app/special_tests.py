# special_tests.py  (피수치 프로젝트 · 특수검사 모듈 / 패치 방식 완전본)
# - 중복 키/ID 완전 차단: 토글/칩/셀렉트박스 모두 섹션+인덱스 기반 유니크 키
# - 탭 가드: 특수탭 외 렌더 방지
# - 렌더 락: 동일 렌더 사이클 내 중복 호출 차단
...
# (필요 시) 외부 저장 경로 등은 다른 모듈에서 관리. 여기서는 UI와 해석만 담당.
# /mnt/data 경로를 직접 쓰지 않음(이 모듈은 상태/입력만 관리하고, 저장은 상위에서 처리)
...
def special_tests_ui() -> List[str]:
    ...
    try:
        ...
        __SP_RENDER_LOCK__ = False  # type: ignore
    if __SP_RENDER_LOCK__:  # type: ignore
        return []
    __SP_RENDER_LOCK__ = True  # type: ignore
...
def _render_urine_panel(lines: List[str]) -> None:
    ...
    with row3[1]:
        sg = st.selectbox("Specific Gravity (비중)", ["1.000", "1.005", "1.010", "1.015", "1.020", "1.025", "1.030"], index=2)

    # 간단 해석 (보호자용)
    tips = []
    if alb != "없음":
        tips.append("알부민뇨(+): 단백뇨가 의심됩니다. **수분 섭취**, **휴식**, 반복 측정을 권장합니다.")
    if glu != "없음":
        tips.append("요당(+): 당대사 이상 가능성. 공복·식후 혈당을 의료진과 상의하세요.")
    if ket != "없음":
        tips.append("케톤(+): **탈수/금식/고열** 시에 나타날 수 있습니다. **수분 보충**을 권장합니다.")
    if blod != "없음":
        tips.append("잠혈(+): **운동·월경·감염** 등 다양한 원인. 반복 측정 및 필요시 소변 현미경 검사가 필요합니다.")
    if nit == "양성" or leu == "양성":
        tips.append("니트라이트/백혈구 양성: **요로감염(UTI)** 가능성. **발열/복통/소변 시 통증**이 있으면 진료가 필요합니다.")
    if sg in ("1.025", "1.030"):
        tips.append("비중 높음: **농축 소변** 가능성. 수분 섭취를 늘려주세요.")
    if sg == "1.000":
        tips.append("비중 낮음: 과도한 수분 섭취 또는 신장기능 이상 의심 시 의료진과 상담하세요.")
...
def _render_urine_mic_panel(lines: List[str]) -> None:
    ...
    tips = []
    if rbc != "0-2":
        tips.append("적혈구 증가: 현미경적 혈뇨. 원인평가(운동/감염/결석 등) 및 재검이 필요할 수 있습니다.")
    if wbc != "0-2":
        tips.append("백혈구 증가: **염증/감염** 시 증가. 증상(발열/배뇨통/빈뇨) 확인이 필요합니다.")
    if cast != "없음":
        tips.append("주형 관찰: **신장 실질 문제** 가능성. 임상과 병합하여 의료진 상담이 필요합니다.")
...
def _render_stool_panel(lines: List[str]) -> None:
    ...
    tips = []
    if ob == "양성":
        tips.append("대변 잠혈 양성: 소화관 출혈 가능성. **검진/상담**이 필요합니다.")
    if c in ("검은색", "붉은색"):
        tips.append("검은/붉은 변: **상/하부 위장관 출혈** 가능성. 증상 동반 시 즉시 진료 필요.")
    if m in ("조금", "많음"):
        tips.append("점액 증가: **염증성 변화** 가능성. 설사·복통 동반 시 의료진 상담을 권장합니다.")

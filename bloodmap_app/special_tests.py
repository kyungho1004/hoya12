...
_RENDER_IDX_KEY = "_special_tests_render_idx_v5"
...
def _ns_prefix() -> str:
    ...
    return f"sp5v1_{MODULE_NS}_r{ridx}"
...
_ALLOWED_UI_ROUTES = {"special", "special_tests", "특수", "특수검사", "dx"}
_REPORT_ROUTES = {"report","reports","export","exports","report_md","export_md","report_pdf"}

def _is_report_context() -> bool:
    route = (st.session_state.get("_route") or "").lower()
    return any(r in route for r in _REPORT_ROUTES)

def _is_ui_context() -> bool:
    route = (st.session_state.get("_route") or "").lower()
    tab = (st.session_state.get("_tab_active") or "").lower()
    if _is_report_context(): return False
    if ("special" in tab) or ("특수" in tab): return True
    if any(route == r for r in _ALLOWED_UI_ROUTES): return True
    if ("special" in route) or ("특수" in route): return True
    if st.session_state.get("_force_special_ui") is True: return True
...
def special_tests_snapshot() -> dict:
    who_raw = st.session_state.get("key", "guest#PIN")
    uid = st.session_state.get("_uid", "")
    snap = {"who": who_raw, "uid": uid, "values": {}, "lines": []}
    fields = {
...
SECTIONS = [
    ...
    ("지질 (TC/TG/HDL/LDL)", "lipid"),
    ("심부전 (BNP/NT-proBNP)", "heartfail"),
    ("당 (FPG/PPG)", "glucose"),
    ("심장/근육 (CK/CK-MB/Troponin)", "cardio"),
    ("간담도 (GGT/ALP)", "hepatobiliary"),
    ("췌장 (Amylase/Lipase)", "pancreas"),
    ("응고 (INR/aPTT/Fibrinogen/D-dimer)", "coag"),
...
def special_tests_ui() -> List[str]:
    ...
    if not _is_ui_context():
        with st.expander("i 특수검사가 비어있나요? (열어서 확인)", expanded=False):
            st.caption("특수검사는 '특수검사' 탭에서 입력/해석할 수 있어요.")
            if st.button("특수검사 강제 로딩", key=_key("fallback", "force_btn")):
                st.session_state["_force_special_ui"] = True
        ...
        for title, sec_id in SECTIONS:
            ...
            if sec_id == "urine":
                ...
                if (v:=upcr) is not None:
                    if v > 10000: _emit(lines, "risk", f"UPCR {v} mg/gCr → 신증후군 범위(극고값). 단위/입력 오류 가능성 고려.")
                ...
                if (v:=acr) is not None:
                    if v > 10000: _emit(lines, "risk", f"ACR {v} mg/gCr → A3(중증) 범위(극고값). 단위/입력 오류 가능성 고려.")
                ...
                if rdw is not None and rdw > 14.5: _emit(lines, "warn", f"RDW {rdw}% ↑ → 크기 불균일")
                if mcv is not None and rdw is not None:
                    if mcv < 80 and rdw > 14.5: _emit(lines, "warn", "소구성 + RDW 증가 → **철결핍** 가능성↑")
                    if mcv < 80 and (rdw <= 14.5): _emit(lines, "warn", "소구성 + RDW 정상 → **지중해 빈혈 보인자** 감별")
                    if mcv > 100 and (ret is not None and ret < 0.5): _emit(lines, "warn", "대구성 + 망상 저하 → **B12/엽산 결핍** 등")
                if ret is not None:
                    if ret >= 2.0: _emit(lines, "warn", f"Reticulocyte {ret}% ↑")
                    elif ret < 0.5: _emit(lines, "warn", f"Reticulocyte {ret}% ↓")
                ...
                if hdl is not None and hdl < 40: _emit(lines, "warn", f"HDL {hdl} 낮음")

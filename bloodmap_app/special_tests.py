...
- 소변검사: 요침사 RBC/WBC, UPCR(단백/Cr), ACR(알부민/Cr) 추가
    ...
    with st.expander("🧪 특수검사 (선택 입력)", expanded=False):
        ...
        for title, sec_id in SECTIONS:
            ...
            if sec_id == "urine":
                ...
                # 정량 지표: UPCR / ACR
                st.caption("정량 지표(Spot urine ratio)")
                q1, q2 = st.columns(2)
                with q1: upcr = _num(st.text_input("단백/Cr 비율 UPCR (mg/gCr)", placeholder="예: 120"))
                with q2: acr  = _num(st.text_input("알부민/Cr 비율 ACR (mg/gCr)", placeholder="예: 25"))

                # 정성 해석
                if alb != "없음":
                    _emit(lines, "warn" if alb in ["+", "++"] else "risk", f"알부민뇨 {alb} → 신장 질환/단백뇨 평가 필요")
                if hem != "없음":
                    _emit(lines, "warn" if hem in ["+", "++"] else "risk", f"혈뇨(잠혈) {hem} → 요로계 출혈/결석/염증 가능성")
                if glu != "없음":
                    _emit(lines, "warn", f"요당 {glu} → 당뇨/신세뇨관 이상 가능, 혈당 확인")
                if nit != "없음":
                ...
                if upcr is not None:
                    if upcr >= 3500:
                        _emit(lines, "risk", f"UPCR {upcr} mg/gCr ≥ 3500 → **신증후군 범위 단백뇨**")
                    elif upcr >= 500:
                        _emit(lines, "warn", f"UPCR {upcr} mg/gCr 500~3499 → **의미 있는 단백뇨**")
                    elif upcr >= 150:
                        _emit(lines, "warn", f"UPCR {upcr} mg/gCr 150~499 → 경도 단백뇨")
                if acr is not None:
                    if acr >= 300:
                        _emit(lines, "risk", f"ACR {acr} mg/gCr ≥ 300 → **중증 알부민뇨**(CKD 고위험)")
                    elif acr >= 30:
                        _emit(lines, "warn", f"ACR {acr} mg/gCr 30~299 → **미세알부민뇨**(초기 신손상 가능)")

...
- 설사 모듈: 빈도 입력 + 안내, 그리고 **시간대별 설사 기록** 테이블 별도 제공
...
def _adult_mg_to_ml(weight: float|None, mgkg: float, cap_low: float, cap_high: float, syrup_mg_per_5ml: float):
...
def render_antipyretic_schedule(storage_key: str = "antipy_sched") -> pd.DataFrame:
    ...
    with st.expander("대상/용량 설정", expanded=True):
        ...
        if diarrhea == "4~6회":
            st.warning("수분/전해질 보충을 충분히 하세요. 증상이 지속되면 의료진과 상의.")
        elif diarrhea == "7회 이상":
    ...
    df_prev = _ensure_df(storage_key, ["No","KST Datetime","대상","나이(개월)","체중(kg)","설사","약물","용량(mL)","용량(정보)","메모"])

    def _append_row(dt_obj, drug, dose_ml, meta_txt, diarrhea_val):
        ...
        new = {
            "No": len(df_prev) + 1,
            "KST Datetime": dt_str,
            "대상": st.session_state.get(f"{storage_key}_who"),
            "나이(개월)": (0 if st.session_state.get(f"{storage_key}_who")=="성인" else int(st.session_state.get(f"{storage_key}_age",0))),
            "체중(kg)": float(st.session_state.get(f"{storage_key}_wt_adult") or st.session_state.get(f"{storage_key}_wt") or 0.0),
            "설사": diarrhea_val,
            "약물": drug,
            "용량(mL)": float(dose_ml),
            "용량(정보)": meta_txt,
    ...
    with b1:
        if st.button("➕ 이 시간으로 추가", key=f"{storage_key}_add_one"):
            ...
            who = st.session_state.get(f"{storage_key}_who")
            apap_ml = st.session_state.get(f"{storage_key}_wt_adult")  # dummy read to ensure keys exist
            # compute dose snapshot again (since locals may be out of scope)
            # (lightweight re-eval using stored values)
            apap_c = float(st.session_state.get(f"{storage_key}_apap_c", 160.0))
            ibu_c  = float(st.session_state.get(f"{storage_key}_ibu_c", 100.0))
            if who == "소아" and callable(acetaminophen_ml) and callable(ibuprofen_ml):
                age_m = int(st.session_state.get(f"{storage_key}_age", 0))
                weight = float(st.session_state.get(f"{storage_key}_wt", 0.0))
                apap_ml_val, _ = acetaminophen_ml(age_m, weight or None)
                ibu_ml_val,  _ = ibuprofen_ml(age_m, weight or None)
            else:
                weight = float(st.session_state.get(f"{storage_key}_wt_adult", 60.0))
                # 성인 계산
                apap_ml_val, apap_mg, _ = _adult_mg_to_ml(weight, 12.5, 325.0, 1000.0, apap_c)
                ibu_ml_val,  ibu_mg,  _ = _adult_mg_to_ml(weight, 7.5, 200.0,  400.0,  ibu_c)
            diarrhea_val = st.session_state.get(f"{storage_key}_diarrhea")
            if agent == "아세트아미노펜":
                ok = _append_row(dt_obj, "아세트아미노펜", apap_ml_val, f"{apap_c} mg/5mL", diarrhea_val)
            elif agent == "이부프로펜":
                ok = _append_row(dt_obj, "이부프로펜", ibu_ml_val, f"{ibu_c} mg/5mL", diarrhea_val)
            else:
                ok = _append_row(dt_obj, "아세트아미노펜", apap_ml_val, f"{apap_c} mg/5mL", diarrhea_val)
                ok = ok or _append_row(dt_obj, "이부프로펜", ibu_ml_val, f"{ibu_c} mg/5mL", diarrhea_val)
            if ok:
    ...
    with st.expander("🔁 연속 스케줄 자동 생성 (선택 기능)", expanded=False):
        ...
        if st.button("➕ 연속 스케줄 생성·추가", key=f"{storage_key}_add_series"):
            ...
            for i in range(int(count)):
                ...
                diarrhea_val = st.session_state.get(f"{storage_key}_diarrhea")
                if agent == "아세트아미노펜":
                    ok = _append_row(dt_i, "아세트아미노펜", apap_ml_val, f"{apap_c} mg/5mL", diarrhea_val)
                elif agent == "이부프로펜":
                    ok = _append_row(dt_i, "이부프로펜", ibu_ml_val, f"{ibu_c} mg/5mL", diarrhea_val)
                else:
                    if i % 2 == 0:
                        ok = _append_row(dt_i, "아세트아미노펜", apap_ml_val, f"{apap_c} mg/5mL", diarrhea_val)
                    else:
    ...
    # --- 설사 기록(시간대) 별도 섹션 ---
    st.markdown("### 💧 설사 기록 (시간대)")
    diar_key = f"{storage_key}_diar_log"
    diar_df = _ensure_df(diar_key, ["No","KST Datetime","횟수(선택)","메모"])
    dc1, dc2, dc3 = st.columns([0.34,0.33,0.33])
    with dc1:
        d_date = st.date_input("날짜 (KST) — 설사", value=_kst_now().date(), key=f"{storage_key}_d_date")
    with dc2:
        d_time = st.time_input("시각 (KST) — 설사", value=_kst_now().time().replace(second=0, microsecond=0), key=f"{storage_key}_d_time")
    with dc3:
        d_freq = st.selectbox("횟수(선택)", ["1회","2회","3회 이상"], key=f"{storage_key}_d_freq")
    d_note = st.text_input("메모(선택) — 설사", key=f"{storage_key}_d_note")
    if st.button("➕ 설사 기록 추가", key=f"{storage_key}_d_add"):
        dt = datetime.combine(d_date, d_time).replace(tzinfo=timezone(timedelta(hours=9)))
        dt_str = dt.strftime("%Y-%m-%d %H:%M")
        # 중복(같은 시각) 방지
        if not diar_df.empty and (diar_df["KST Datetime"] == dt_str).any():
            st.info("이미 같은 시각의 설사 기록이 있습니다.")
        else:
            diar_df.loc[len(diar_df)] = {"No": len(diar_df)+1, "KST Datetime": dt_str, "횟수(선택)": d_freq, "메모": d_note}
            diar_df = diar_df.sort_values("KST Datetime").reset_index(drop=True)
            diar_df["No"] = range(1, len(diar_df)+1)
            _save_df(diar_key, diar_df)
            st.success("설사 기록 1건 추가됨.")
    if diar_df.empty:
        st.info("아직 설사 기록이 없습니다.")
    else:
        st.dataframe(diar_df, use_container_width=True, height=220)
        st.download_button("⬇️ 설사 기록 CSV", data=diar_df.to_csv(index=False).encode("utf-8"), file_name="diarrhea_log_kst.csv")

    # --- 스케줄 표시/삭제 ---
    df = _ensure_df(storage_key, ["No","KST Datetime","대상","나이(개월)","체중(kg)","설사","약물","용량(mL)","용량(정보)","메모"])
    if df.empty:
    ...
    st.download_button("⬇️ 스케줄 CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="antipyretic_schedule_kst.csv")
    return df

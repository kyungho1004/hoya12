...
def render_onco_antipyretic_log(storage_key: str = "onco_antipyretic_log"):
    ...
    with st.expander("⚙️ 고급 설정 (농도·mg/kg 조절)", expanded=False):
        ...
        with s1: apap_c = st.number_input("아세트아미노펜(APAP) 농도 (mg/5mL)", min_value=80.0, max_value=500.0, step=10.0, value=160.0, key=f"{storage_key}_apap_c")
        with s2: ibu_c  = st.number_input("이부프로펜(IBU) 농도 (mg/5mL)",  min_value=50.0, max_value=400.0, step=10.0, value=100.0, key=f"{storage_key}_ibu_c")
        with s3: apap_mgkg = st.number_input("아세트아미노펜(APAP) mg/kg", min_value=8.0, max_value=15.0, step=0.5, value=12.5, key=f"{storage_key}_apap_mgkg")
        with s4: ibu_mgkg  = st.number_input("이부프로펜(IBU) mg/kg",  min_value=5.0, max_value=10.0, step=0.5, value=7.5,  key=f"{storage_key}_ibu_mgkg")
    ...
    with cA:
        ...
        if st.button("➕ 아세트아미노펜(APAP) 기록", use_container_width=True, key=f"{storage_key}_add_apap"):
    ...
    with cB:
        ...
        if st.button("➕ 이부프로펜(IBU) 기록", use_container_width=True, key=f"{storage_key}_add_ibu"):
            dt = datetime.combine(d, t).replace(tzinfo=timezone(timedelta(hours=9)))
            df = _ensure_df(storage_key)
            row = {"KST_time": dt.strftime("%Y-%m-%d %H:%M"), "Who": who, "Age_m": (int(age_m) if age_m is not None else ""),

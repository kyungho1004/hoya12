...
# --- Patch: cancer → regimen map with maintenance ---
# 키는 내부 진단명(영문 우선), 값은 phase별 약물 리스트
ONCO_REGIMENS = {
    # Acute lymphoblastic leukemia (ALL)
    "ALL": {
        "induction": ["Vincristine", "Prednisone", "Daunorubicin", "L-asparaginase"],
        "consolidation": ["Cytarabine", "Cyclophosphamide", "6-MP", "MTX"],
        "maintenance": ["6-MP", "MTX"]  # 간단화(가정용 안내): 핵심 2축
    },
    # Acute promyelocytic leukemia (APL)
    "APL": {
        "induction": ["ATRA", "Arsenic Trioxide"],
        "consolidation": ["ATRA", "Arsenic Trioxide", "Idarubicin"],
        # 사용자 요구 고정: APL 유지요법에 MTX/6-MP 포함
        "maintenance": ["ATRA", "6-MP", "MTX"]
    },
    # Acute myeloid leukemia (AML, non-APL)
    "AML": {
        "induction": ["Cytarabine", "Idarubicin"],
        "consolidation": ["Cytarabine (HDAC)"],
        "maintenance": []  # 표준 유지요법 없음(단순 고지)
    },
    # Diffuse Large B-Cell Lymphoma
    "DLBCL": {
        "induction": ["Rituximab", "Cyclophosphamide", "Doxorubicin", "Vincristine", "Prednisone"],  # R-CHOP
        "consolidation": [],
        "maintenance": []  # 일부 상황에서 리툭시맙 유지 고려되나 가정용 기본값은 공란
    },
    # CML (만성기)
    "CML": {
        "induction": ["Imatinib"],
        "consolidation": [],
        "maintenance": ["Imatinib"]  # TKI 지속 복용
    },
    # HER2+ Breast cancer (adjuvant/advanced 간단 버전)
    "Breast_HER2pos": {
        "induction": ["Docetaxel", "Carboplatin", "Trastuzumab", "Pertuzumab"],
        "consolidation": ["Trastuzumab"],
        "maintenance": ["Trastuzumab", "T-DM1"]  # 상황 따라 선택(간단 표기)
    },
    # EGFRm NSCLC
    "NSCLC_EGFR": {
    ...
    "NSCLC_ALK": {
        "induction": ["Alectinib"],
        "consolidation": [],
        "maintenance": ["Alectinib"]
    },
    # Colorectal (RAS WT)
    "CRC_RAS_WT": {
        "induction": ["5-FU", "Oxaliplatin", "Leucovorin", "Cetuximab"],
        "consolidation": ["5-FU"],
        "maintenance": ["5-FU"]
    },
}

# 유지: 기존 맵이 있다면 병합(패치 원칙)
try:
    EXISTING = ONCO_REGIMENS  # if file already had, we keep reference
except Exception:
    EXISTING = {}

def ensure_onco_map(m: dict):
    if m is None:
        return ONCO_REGIMENS
    # 병합: 신규 키 추가, 기존 키는 덮지 않음
    for k, phases in ONCO_REGIMENS.items():
        if k not in m:
            m[k] = phases
        else:
            # phase 병합
...
__all__ = ["ONCO_REGIMENS", "ensure_onco_map"]

...
def auto_recs_by_dx(group: str, dx: str, DRUG_DB: Dict[str, Dict[str, Any]] = None,
    ...
    """
    Return drug lists for the selected diagnosis.
    (No default antibiotics; only what the map contains.)
    """
    ...
    for k in out.keys():
        picks = dmap.get(k, [])
        # filter by DRUG_DB if provided; otherwise keep raw keys
        if DRUG_DB:
            out[k] = [p for p in picks if p in DRUG_DB]
        else:
            out[k] = list(picks)
    return out

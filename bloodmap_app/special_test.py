...
def special_tests_snapshot() -> dict:
    """현재 세션 값을 스냅샷으로 저장하고 special_tests_lines 캐시 보존."""
    import streamlit as st
...
def special_section_md(lines: list[str] | None = None, show_empty: bool = True) -> str:
    """라인이 비어도 헤더는 유지하는 MD 생성기(보고서/PDF 공용)."""
    import streamlit as st
    if lines is None:
    ...
    if lines:
        ...
        md.append("- (입력된 특수검사 요약이 없습니다)")
    return "\n".join(md)

def special_section() -> str:
    """기존 API 호환: 내부에서 스냅샷 보장 + 빈 섹션도 생성."""
    import streamlit as st
    if not st.session_state.get("special_tests_lines"):
        try:
            ...
            pass
    return special_section_md(show_empty=True)

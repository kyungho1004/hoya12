...
# --- 모니터링 항목 필터(사용자 혼동 방지) ---
EXCLUDED_MONITOR_TERMS = {"bnp", "nt-probnp", "nt probnp", "nt pro bnp"}

def _filter_monitor_items(items):
    """Hide advanced/ambiguous items like BNP/NT-proBNP from patient-facing UI."""
    out = []
    for it in items or []:
        s = str(it)
        s_low = s.lower()
        if any(term in s_low for term in EXCLUDED_MONITOR_TERMS):
            continue
        out.append(s)
    return out

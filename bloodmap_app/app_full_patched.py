...
def _parse_diarrhea_count(label: str) -> int:
    s = str(label or "")
    if "7" in s: return 7
...
# ---------------- 일상 모드 ----------------
    ...
    if who == "소아":
        ...
        eye = st.selectbox("눈꼽", opts.get("눈꼽", ["없음","맑음","노랑-농성","가려움 동반","한쪽","양쪽"]))
        ...
        triage = triage_advise(temp, age_m, _parse_diarrhea_count(diarrhea))
        st.info(triage)
